...
describe("mcp-auth storage paths", () => {
  const originalEnv = {
    ...
    PI_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY:
      process.env.PI_MCP_ADAPTER_TEST_LINUX_KEYRING_RECOVERY,
    PI_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL:
      process.env.PI_MCP_ADAPTER_KEYRING_RECOVERY_KEYCTL,
    PI_MCP_ADAPTER_KEYRING_RECOVERY_NODE:
      process.env.PI_MCP_ADAPTER_KEYRING_RECOVERY_NODE,
    PI_MCP_ADAPTER_KEYRING_RECOVERY_HELPER:
      process.env.PI_MCP_ADAPTER_KEYRING_RECOVERY_HELPER,
    PI_MCP_ADAPTER_FAKE_KEYRING_STORE:
      process.env.PI_MCP_ADAPTER_FAKE_KEYRING_STORE,
  };
  let authDir: string;
  ...
  it("keeps arbitrary configured server names under safe hashed legacy import paths", () => {
    ...
    expect(existsSync(join(authDir, "..", "escape", "tokens.json"))).toBe(
      false,
    );
  });
  ...
  it("uses configured oauthDir as the legacy import source", () => {
    ...
    expect(getAuthEntry("configured", options)?.tokens?.accessToken).toBe(
      "legacy-token",
    );
  ...
  it("does not migrate legacy credentials during status-only inspection", () => {
    ...
    expect(
      inspectAuthForUrl("status-only", "https://example.com/mcp").status,
    ).toBe("present");
    expect(existsSync(filePath)).toBe(true);

    expect(getAuthEntry("status-only")?.tokens?.accessToken).toBe(
      "legacy-token",
    );
    expect(existsSync(filePath)).toBe(false);
  });
  ...
  it("does not use configured oauthDir values as secure-store namespaces", () => {
    ...
    expect(getAuthEntry("same-server", optionsA)?.tokens?.accessToken).toBe(
      "token-b",
    );
    expect(getAuthEntry("same-server", optionsB)?.tokens?.accessToken).toBe(
      "token-b",
    );
  ...
  it("chunks large secure-store entries and reads them back", () => {
    ...
    const manifestEntry = entries.find(
      ([account]) => !account.includes(".chunk."),
    );
    const chunkEntries = entries.filter(([account]) =>
      account.includes(".chunk."),
    );

    expect(manifestEntry).toBeDefined();
    const manifest = JSON.parse(manifestEntry![1]) as {
      __piMcpAdapterOAuthChunked?: number;
      chunkCount?: number;
    };
    ...
    expect(
      chunkEntries.every(
        ([, payload]) => payload.length <= AUTH_SECRET_VALUE_LIMIT,
      ),
    ).toBe(true);
  });
  ...
  it("persists records that exceed the strictest per-value store limit", () => {
    ...
    expect(getAuthEntry("size-limited-large")?.tokens?.accessToken).toBe(
      accessToken,
    );
    expect(
      getTestAuthSecretStoreEntries().every(
        ([, payload]) => payload.length <= AUTH_SECRET_VALUE_LIMIT,
      ),
    ).toBe(true);
  });
  ...
  it("persists records just above the per-value limit that are too small for a naive chunk threshold", () => {
    ...
    saveAuthEntry(
      "size-limited-boundary",
      { tokens: { accessToken } },
      "https://example.com/mcp",
    );

    expect(getAuthEntry("size-limited-boundary")?.tokens?.accessToken).toBe(
      accessToken,
    );
    expect(
      getTestAuthSecretStoreEntries().every(
  ...
  it("keeps small records in a single entry on a size-limited store", () => {
    ...
    saveAuthEntry(
      "size-limited-small",
      { tokens: { accessToken: "small" } },
      "https://example.com/mcp",
    );

    expect(getAuthEntry("size-limited-small")?.tokens?.accessToken).toBe(
      "small",
    );
    const entries = getTestAuthSecretStoreEntries();
  ...
  it("clears chunked records written to a size-limited store", () => {
    ...
    saveAuthEntry(
      "size-limited-remove",
      { tokens: { accessToken: "x".repeat(5000) } },
      "https://example.com/mcp",
    );
    expect(
      getTestAuthSecretStoreEntries().some(([account]) =>
        account.includes(".chunk."),
      ),
    ).toBe(true);
  ...
  it("returns unavailable status when a stored chunk cannot be read", () => {
    ...
    const chunkAccount = getTestAuthSecretStoreEntries().find(([account]) =>
      account.includes(".chunk."),
    )?.[0];
    ...
    expect(
      inspectAuthForUrl("large-status", "https://example.com/mcp").status,
    ).toBe("unavailable");
  });
  ...
  it("removes chunk payloads when credentials are cleared", () => {
    ...
    const storedAccounts = getTestAuthSecretStoreEntries().map(
      ([account]) => account,
    );
    expect(storedAccounts.some((account) => account.includes(".chunk."))).toBe(
      true,
    );
    ...
    const remainingAccounts = new Set(
      getTestAuthSecretStoreEntries().map(([account]) => account),
    );
    expect(
      storedAccounts.every((account) => !remainingAccounts.has(account)),
    ).toBe(true);
  });
...
`,
  ...
  it("does not use the recovery helper for generic secure-store failures", () => {
    const harnessDir = mkdtempSync(
      join(tmpdir(), "pi-mcp-keyring-no-recovery-"),
    );
    const keyctlPath = join(harnessDir, "keyctl");
    const storePath = join(harnessDir, "store.json");
    ...
    expect(() => getAuthEntry("generic-unavailable")).toThrow(
