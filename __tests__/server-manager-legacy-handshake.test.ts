...
const legacyFixture = fileURLToPath(
  new URL("./fixtures/legacy-no-discover-server.mjs", import.meta.url),
);
const legacyExitsOnDiscoverFixture = fileURLToPath(
  new URL("./fixtures/legacy-exits-on-discover-server.mjs", import.meta.url),
);
const modernFixture = fileURLToPath(
  new URL("./fixtures/modern-discover-server.mjs", import.meta.url),
);
...
describe("McpServerManager protocol negotiation", () => {
  it("defaults to the classic initialize handshake without server/discover", async () => {
    ...
    expect(connection.client.getNegotiatedProtocolVersion()).not.toBe(
      "2026-07-28",
    );
    expect(connection.tools.map((tool) => tool.name)).toEqual([
      "classic_initialize_reached",
    ]);
  }, 5_000);
  ...
  it("keeps traced auto negotiation on the SDK disposable stdio probe path", async () => {
    ...
    expect(connection.tools.map((tool) => tool.name)).toEqual([
      "legacy_after_sibling_probe",
    ]);
  }, 5_000);

  it.each(["auto", "2026-07-28"] as const)(
    "connects to a modern stdio server in %s mode",
