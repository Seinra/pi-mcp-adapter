...
function httpError(status: number, message: string): SdkHttpError {
  return new SdkHttpError(SdkErrorCode.ClientHttpNotImplemented, message, {
    status,
  });
}
    ...
    manager.setMetadataListChangedListener((_serverName, reason) => {
      if (reason === "session-reconnect") {
        publishedCatalogs.push(
          manager.getConnection("demo")?.tools.map((tool) => tool.name) ?? [],
        );
      }
    });
      ...
      const result = await withSessionRecovery(
        ...
        (connection) => {
          ...
          if (recoveryAttempts === 2) {
            replaySawPublishedCatalog = publishedCatalogs.some((tools) =>
  ...
  it("polls without a notification stream and reconnects when that session later expires", async () => {
    ...
    const server = createServer(async (req, res) => {
      ...
      if (message.method === "tools/list") {
        ...
        if (
          rejectStaleSession &&
          requestSessionId !== `session-${generation}`
        ) {
    ...
    try {
      ...
      expect(staleConnection.tools.map((tool) => tool.name)).toEqual([
        "old_tool",
      ]);
      ...
      expect(freshConnection?.tools.map((tool) => tool.name)).toEqual([
        "new_tool",
      ]);
  ...
  it("recovers a terminated Streamable HTTP session transparently mid tool-call", async () => {
    ...
    const stale = {
      ...
      client: {
        callTool: vi
          .fn()
          .mockRejectedValueOnce(httpError(404, "Session not found")),
      },
    };
    const fresh = {
      status: "connected" as const,
    ...
    const state = {
      config: {
        settings: {},
        mcpServers: { demo: { url: "https://api.example.com/mcp" } },
      },
      manager,
      ...
      client: {
        callTool: vi
          ...
          .mockRejectedValueOnce(
            new ProtocolError(-32000, "Server not initialized"),
          ),
      },
    };
    const fresh = {
  ...
  it("gives up after one reconnect attempt: a second terminated session propagates as call_failed", async () => {
    ...
    const stale = {
      ...
      client: {
        callTool: vi
          .fn()
          .mockRejectedValue(httpError(404, "Session not found")),
      },
    };
    const fresh = {
      status: "connected" as const,
