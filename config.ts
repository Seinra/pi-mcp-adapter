...
const GENERIC_GLOBAL_CONFIG_PATH = join(
  homedir(),
  ".config",
  "mcp",
  "mcp.json",
...
export function getMcpStandardConfigSummary(
  overridePath?: string,
  cwd = process.cwd(),
...
export function getMcpDiscoverySummary(
    ...
    ? importKinds
        .map((kind) => {
          ...
          return {
            kind,
            path: imported.path,
            serverCount: Object.keys(extractServers(imported.value, kind))
              .length,
  ...
  const hostConfigs = imports.map((entry) => ({
    ...entry,
    active: hostConfigDiscovery === "on",
  }));
  ...
  const totalServerCount =
    sources.reduce((sum, source) => sum + source.serverCount, 0) +
    agentPlugins.reduce((sum, plugin) => sum + plugin.serverCount, 0);
  const hasSharedServers =
    sources.some(
      (source) => source.kind === "shared" && source.serverCount > 0,
  ...
  const hasAnyDetectedPaths =
    sources.some((source) => source.exists) ||
    imports.length > 0 ||
    agentPlugins.length > 0;
  const hasAnyConfig =
    totalServerCount > 0 ||
    imports.some((entry) => entry.serverCount > 0) ||
    hasAnyDetectedPaths;
  ...
  const fingerprint = JSON.stringify({
    ...
    agentPlugins: agentPlugins.map((entry) => [
      entry.path,
      entry.name,
      entry.serverCount,
    ]),
...
export function loadMcpConfig(
  ...
  const sourceSpecs = getConfigSources(overridePath, cwd);
  const hostConfigDiscovery = getConfiguredHostConfigDiscovery(
    overridePath,
    cwd,
  );
  ...
  let config: McpConfig =
    !isExclusiveConfigMode() && hostConfigDiscovery === "on"
  ...
  const pluginConfig = loadAgentPluginConfigs(
    config.settings?.agentPluginPaths,
    cwd,
  );
  const packageServers = Object.fromEntries(
    Object.entries(packageConfig.mcpServers).filter(
...
function getConfigConflicts(
  ...
  const seen = new Map<
    string,
    Array<{ kind: "shared" | "pi" | "host"; path: string }>
  >();
  const record = (
    name: string,
    source: { kind: "shared" | "pi" | "host"; path: string },
  ): void => {
    const entries = seen.get(name) ?? [];
    if (
      !entries.some(
        (entry) => entry.kind === source.kind && entry.path === source.path,
      )
    )
      entries.push(source);
  ...
  for (const entry of imports) {
    ...
    for (const name of Object.keys(
      extractServers(imported.value, entry.kind),
    )) {
  ...
  for (const source of sourceSpecs) {
    ...
    if (loaded.imports?.length) {
      for (const importKind of loaded.imports) {
        ...
        if (!imported) continue;
        for (const name of Object.keys(
          extractServers(imported.value, importKind),
        )) {
          record(name, { kind: "host", path: imported.path });
        }
      }
...
function expandImports(config: McpConfig, cwd = process.cwd()): McpConfig {
  ...
  for (const importKind of config.imports) {
    const imported = loadImportedConfig(
      importKind,
      cwd,
      `Failed to import MCP config from ${importKind}:`,
    );
    if (!imported) continue;
...
function loadImportedConfig(
  ...
  if (importKind === "opencode") {
    ...
    return highestPrecedencePath
      ? { path: highestPrecedencePath, value: merged }
      : null;
  }
...
function validateConfig(raw: unknown): McpConfig {
  ...
  return {
    ...
    ...(Array.isArray(raw.imports)
      ? { imports: raw.imports as ImportKind[] }
      : {}),
...
function mergeOpenCodeConfigs(
  ...
  const mergedMcp: Record<string, unknown> = {
    ...(baseMcp && typeof baseMcp === "object" && !Array.isArray(baseMcp)
      ? baseMcp
      : {}),
  };
      ...
      if (
        ...
        if (Array.isArray(override.command)) {
          ...
          const commandChanged =
            ...
            override.command.some(
              (value, index) => value !== baseCommand[index],
            );
          if (commandChanged) {
...
function extractServers(
  ...
  for (const [name, entry] of Object.entries(servers)) {
      ...
      if (raw.type === "remote" && typeof raw.url === "string") {
        ...
        if (raw.oauth === false) {
          ...
          mapped.oauth = {
            ...
            ...(typeof oauth.authServerMetadataUrl === "string"
            ...
            ...(typeof oauth.skipIssuerMetadataValidation === "boolean"
              ? {
                  skipIssuerMetadataValidation:
                    oauth.skipIssuerMetadataValidation,
                }
              : {}),
          };
        }
    ...
    if (
      httpHeaders &&
      typeof httpHeaders === "object" &&
      !Array.isArray(httpHeaders)
    ) {
      mapped.headers = {
...
function buildUnifiedDiff(beforeText: string, afterText: string): string {
  ...
  while (i < rows || j < cols) {
    ...
    if (
      j < cols &&
      (i === rows || (lcs[i]?.[j + 1] ?? 0) >= (lcs[i + 1]?.[j] ?? 0))
    ) {
      lines.push(`+ ${after[j]}`);
      j++;
      continue;
...
export function writeProjectServerDisabledOverride(
  ...
  if (disabled) {
    ...
    next = Object.fromEntries(
      Object.entries(existing ?? {}).filter(([key]) => key !== "disabled"),
    );
    ...
    if (raw.imports !== undefined) {
      if (
        ...
        raw.imports.some(
          (kind) =>
            typeof kind !== "string" || !Object.hasOwn(IMPORT_PATHS, kind),
        )
      ) {
        throw new Error(
      ...
      lowerConfig = mergeConfigs(
        lowerConfig,
        expandImports(
          { mcpServers: {}, imports: raw.imports as ImportKind[] },
          cwd,
        ),
      );
    }
...
function isRepoPromptServer(name: string, entry: ServerEntry): boolean {
  ...
  return (entry.args ?? []).some(
    (arg) =>
      typeof arg === "string" && arg.toLowerCase().includes("repoprompt"),
  );
}
...
function detectRepoPrompt(
  ...
  for (const source of summary.sources) {
    ...
    const config = readValidatedConfig(
      source.path,
      `MCP config from ${source.path}`,
    );
    if (!config) continue;
...
export function previewCompatibilityImports(
  ...
  const currentImports = Array.isArray(raw.imports)
    ? raw.imports.filter(
        (value): value is ImportKind => typeof value === "string",
      )
    : [];
...
export function previewStarterSharedConfig(
  target: SharedConfigTarget,
  cwd = process.cwd(),
): ConfigWritePreview {
...
export function writeStarterSharedConfig(
  target: SharedConfigTarget,
  cwd = process.cwd(),
): string {
  const targetPath = getSharedConfigPath(target, cwd);
...
export function previewStarterProjectConfig(
  cwd = process.cwd(),
): ConfigWritePreview {
...
export function resolveConfiguredOAuthDir(
  raw: unknown,
  cwd = process.cwd(),
): string | undefined {
