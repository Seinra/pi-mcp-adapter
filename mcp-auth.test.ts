...
const TEST_DIR = join(
  tmpdir(),
  `mcp-oauth-test-${randomBytes(4).toString("hex")}`,
);
...
describe("mcp-auth", () => {
  ...
  describe("keyring native binding fallback", () => {
      ...
      const requireStub = Object.assign(
        ...
        },
        {
          resolve(id: string) {
            assert.strictEqual(
              id,
              "@napi-rs/keyring-darwin-arm64/package.json",
            );
    ...
    it("tries the Linux musl package when the gnu package is unavailable", () => {
      const loaderError = new Error("package loader failed");
      const nativePath =
        "/tmp/keyring-linux-x64-musl/keyring.linux-x64-musl.node";
      ...
      const requireStub = Object.assign(
        ...
        },
        {
          resolve(id: string) {
            resolved.push(id);
            if (id === "@napi-rs/keyring-linux-x64-musl/package.json")
    ...
    it("keeps the original loader error in the cause chain when fallback fails", () => {
      ...
      assert.throws(
        () => loadTestKeyringEntryClass(requireStub, "darwin", "arm64"),
        (error) => {
          assert(error instanceof Error);
          assert.match(
            error.message,
            /absolute-path native binding fallback also failed: native binding failed/,
          );
          ...
          assert.fail(
            "original loader error was not preserved in the cause chain",
          );
        },
      );
    });
  ...
  describe("getAuthEntry", () => {
    ...
    it("should import legacy plaintext entries and remove the file", () => {
      ...
      assert.strictEqual(
        getAuthEntry("legacy-import")?.tokens?.accessToken,
        "legacy-token",
      );
    });
  ...
  describe("updateTokens", () => {
    ...
    it("should clear URL-bound auth state when tokens move to a different server URL", () => {
      ...
      assert.strictEqual(
        getAuthForUrl("token-url-change", "https://old.example.com/mcp"),
        undefined,
      );
      const newEntry = getAuthForUrl(
        "token-url-change",
        "https://new.example.com/mcp",
      );
  ...
  describe("updateClientInfo", () => {
    ...
    it("should clear URL-bound credentials when client info moves to a different server URL", () => {
      ...
      assert.strictEqual(
        getAuthForUrl("url-change", "https://old.example.com/mcp"),
        undefined,
      );
      const newEntry = getAuthForUrl(
        "url-change",
