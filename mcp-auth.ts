...
const TEST_LINUX_KEYRING_RECOVERY_ENV =
...
export function formatOAuthCredentialStoreUnavailable(
  error: OAuthCredentialStoreError,
): string {
  if (
...
const sizeLimitedAuthSecretStore: AuthSecretStore = {
  ...
  write(account, payload) {
    if (payload.length > AUTH_SECRET_VALUE_LIMIT) {
      throw new Error(
        `Value of 'password encoded as UTF-16' is longer than the platform limit of ${AUTH_SECRET_VALUE_LIMIT * 2} chars`,
      );
    }
    memoryAuthEntries.set(account, payload);
  },
...
function createKeyRevokedTestError(): Error {
  return new Error("Couldn't access platform storage: KeyRevoked", {
...
function loadKeyringNativeBindingFallback(
  ...
  if (targets.length === 0) {
    throw new Error(
      `Unsupported @napi-rs/keyring native binding target: ${platform}-${arch}`,
    );
  }
  ...
  for (const target of targets) {
    try {
      const packageJsonPath = keyringRequire.resolve(
        `${target.packageName}/package.json`,
      );
      return keyringRequire(
        join(dirname(packageJsonPath), target.bindingFile),
      ) as KeyringModule;
    } catch (error) {
      lastError = error;
    }
...
function getKeyringNativeBindingSuffixes(
  platform: NodeJS.Platform,
  arch: NodeJS.Architecture,
): string[] {
  if (platform === "darwin") {
...
function isLinuxKeyringRecoveryEnabled(): boolean {
  ...
  return (
    process.platform === "linux" ||
    process.env[TEST_LINUX_KEYRING_RECOVERY_ENV] === "1"
  );
}
...
function runLinuxKeyringRecoveryOperation(
  ...
  if (result.error) {
    throw new Error(
      `Linux keyring recovery helper could not start: ${result.error.message}`,
      { cause: result.error },
    );
  }
  if (result.status !== 0) {
    throw new Error(
      `Linux keyring recovery helper failed with exit code ${result.status ?? "unknown"}`,
    );
  }
  ...
  try {
    ...
    throw new Error("Linux keyring recovery helper returned invalid JSON", {
      cause: error,
    });
  }
  if (
    ...
    throw new Error(
      "Linux keyring recovery helper returned an invalid response",
    );
  }
  const typedResponse = response as KeyringRecoveryResponse;
  if (typedResponse.ok === false) {
    throw new Error(
      typedResponse.error || "Linux keyring recovery helper failed",
    );
  }
  if (
    operation === "read" &&
 ...
 */
export function getAuthEntryFilePath(
  serverName: string,
  options?: AuthStorageOptions,
): string {
...
function parseJsonPayload(
  ...
  try {
    ...
    throw new Error(
      `Failed to parse OAuth credentials for ${serverName} from ${source}`,
      { cause: error },
    );
  }
}

function parseAuthEntryPayload(
  serverName: string,
  payload: string,
  source: string,
): AuthEntry {
  ...
  if (!entry) {
    throw new Error(
      `Failed to parse OAuth credentials for ${serverName} from ${source}: invalid credential shape`,
    );
  }
  return entry;
}
...
function toAuthEntry(value: unknown): AuthEntry | undefined {
  ...
  const tokens =
    entry.tokens === undefined ? undefined : toStoredTokens(entry.tokens);
  const clientInfo =
    entry.clientInfo === undefined
      ? undefined
      : toStoredClientInfo(entry.clientInfo);
  if (
...
function toStoredClientInfo(value: unknown): StoredClientInfo | undefined {
  ...
  const clientSecretExpiresAt = optionalNumber(
...
function stringArray(value: unknown): string[] | undefined {
  return Array.isArray(value) && value.every((uri) => typeof uri === "string")
    ? value
    : undefined;
}

function isAuthEntryChunkManifest(
  value: unknown,
): value is AuthEntryChunkManifest {
...
function getAuthEntryChunkAccount(
  account: string,
...
function getAuthEntryChunkAccounts(
  account: string,
  manifest: AuthEntryChunkManifest,
): string[] {
...
function readChunkedAuthEntry(
  ...
  const chunks = getAuthEntryChunkAccounts(account, manifest).map(
    (chunkAccount) => {
      try {
        const chunk = store.read(chunkAccount);
        if (chunk === undefined) {
          throw new Error(
            `Missing OAuth credential chunk ${chunkAccount} for ${serverName}`,
          );
        }
        return chunk;
      } catch (error) {
  ...
  return parseAuthEntryPayload(
    serverName,
    chunks.join(""),
    "OS secure credential store chunks",
  );
}
...
function removeLegacyAuthEntry(
  ...
  try {
    ...
    throw new Error(
      `Failed to remove legacy plaintext OAuth credentials for ${serverName} at ${filePath}`,
      { cause: error },
    );
  }
...
function writeSecureAuthEntryToStore(
  ...
  const previousManifest = readExistingChunkManifest(
    store,
    serverName,
    account,
  );
...
function readAuthEntryFromStore(
  ...
  if (payload !== undefined) {
    const manifest = readChunkManifestFromPayload(
      serverName,
      payload,
      "OS secure credential store",
    );
...
export function inspectAuthForUrl(
  ...
  try {
    ...
    return {
      status: "unavailable",
      message: formatOAuthCredentialStoreUnavailable(error),
    };
  }
}
 ...
 */
function removeAuthEntryFromStore(
  store: AuthSecretStore,
  serverName: string,
): void {
  const account = getAuthEntryAccount(serverName);
...
export function removeAuthEntry(
  serverName: string,
  options?: AuthStorageOptions,
): void {
  try {
