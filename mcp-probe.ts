...
async function getJsonRpcEnvelopeInfo(
  response: Response,
): Promise<JsonRpcEnvelopeInfo | null> {
  try {
...
async function classifyResponse(
  ...
  const envelope =
    strategy.allowJson || response.status === 401
      ? await getJsonRpcEnvelopeInfo(response)
      : null;
...
export async function probeMcpEndpoint(
  ...
  const { response: modernResponse, outcome: modernOutcome } = await probe(
    url,
    MODERN_STRATEGY,
  );
  ...
  const { response: postResponse, outcome: postOutcome } = await probe(
    url,
    LEGACY_POST_STRATEGY,
  );
  if (postOutcome.kind === "mcp") return postOutcome.result;
  ambiguousResult ??= ambiguousNotMcp(postResponse);
  if (!POST_ENDPOINT_MISMATCH_STATUSES.has(postResponse.status))
    return (
      ambiguousNotMcp(postResponse) ?? ambiguousResult ?? notMcp(postResponse)
    );

  const { response: getResponse, outcome: getOutcome } = await probe(
    url,
    LEGACY_SSE_STRATEGY,
  );
  return getOutcome.kind === "mcp"
    ? getOutcome.result
    : (ambiguousNotMcp(getResponse) ?? ambiguousResult ?? notMcp(getResponse));
}
