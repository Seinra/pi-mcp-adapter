...
import { MODERN_PROTOCOL_VERSION, probeMcpEndpoint } from "./mcp-probe.ts";
import {
  isServerDisabled,
  type McpTool,
  type McpResource,
...
/**
 * Legacy is the default: undefined/"legacy" sends no negotiation option
 * (classic initialize, no server/discover probe). "auto" and "2026-07-28"
 * are explicit opt-ins to modern negotiation or a hard pin.
 */
...
function boundedStderrChunk(chunk: Buffer | string): Buffer {
  ...
  const suffix =
    chunk.length > MAX_CAPTURED_STDERR_BYTES
...
function appendStderrTail(tail: Buffer, chunk: Buffer | string): Buffer {
  ...
  return combined.length > MAX_CAPTURED_STDERR_BYTES
    ? Buffer.from(
        combined.subarray(combined.length - MAX_CAPTURED_STDERR_BYTES),
      )
    : combined;
}
...
export interface ServerConnection {
  ...
  toolListHints?:
    | Partial<Pick<ListToolsResult, "ttlMs" | "cacheScope">>
    | undefined;
...
type ListenStateChangedListener = (
  serverName: string,
...
type ToolListCacheHints = Partial<
  Pick<ListToolsResult, "ttlMs" | "cacheScope">
>;
...
export class McpServerManager {
  ...
  private resourceUpdatedListeners = new Map<
    string,
    {
      serverName: string;
      uri: string;
  ...
  setMetadataListChangedListener(
    listener: MetadataListChangedListener | undefined,
  ): void {
    this.metadataListChangedListener = listener;
  }

  setListenStateChangedListener(
    listener: ListenStateChangedListener | undefined,
  ): void {
  ...
  publishMetadataChanged(
    ...
    try {
      ...
      this.pendingMetadataPublications.set(name, {
        connection: expectedConnection,
        reason,
      });
      const message = error instanceof Error ? error.message : String(error);
      logger.debug(
        `MCP: metadata publication failed for ${name}; queued for retry: ${message}`,
      );
      return false;
    }
  }
  ...
  async connect(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `MCP connection for ${name} was closed while connecting`,
        );
      }
      this.connections.set(name, connection);
      this.watchListenSubscription(
        name,
        connection,
        connection.listenSubscription,
      );
      if (
        [...this.resourceUpdatedListeners.values()].some(
          (registration) => registration.serverName === name,
        )
      ) {
  ...
  private currentListenFilter(
    ...
    const recentResourceUris =
      recentSlots > 0
        ? [...(recent?.keys() ?? [])]
            .filter((uri) => !openSet.has(uri))
            .slice(-recentSlots)
        : [];
  ...
  async ensureListen(
    ...
    const sameFilter = isDeepStrictEqual(
      expectedConnection.listenFilter,
      filter,
    );
    if (expectedConnection.listenState === "active" && sameFilter) {
      ...
      const attempt = (async () => {
        ...
        const confirmed = await this.reconcileCatalogAfterListen(
          name,
          expectedConnection,
          this.buildRequestOptions(expectedConnection.definition),
        );
        if (!confirmed)
          expectedConnection.listenRetryAfter =
            Date.now() + LISTEN_RETRY_DELAY_MS;
    ...
    const attempt = (async () => {
      ...
      try {
        ...
        const confirmed = recoverDroppedListen
          ? await this.reconcileCatalogAfterListen(
              name,
              expectedConnection,
              requestOptions,
            )
        ...
        logger.debug(
          `MCP: catalog listen repair failed for ${name}: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    })().finally(() => {
      if (expectedConnection.listenPromise === attempt)
  ...
  private async reconcileCatalogAfterListen(
    ...
    const nextTools =
      toolResult.status === "fulfilled" ? toolResult.value : undefined;
    const nextResources =
      resources.status === "fulfilled" ? resources.value : undefined;
    const nextPrompts =
      promptResult.status === "fulfilled" && !promptResult.value.failed
        ? promptResult.value
        : undefined;
    const confirmed =
      nextTools !== undefined &&
    ...
    const changed =
      (nextTools !== undefined &&
        (!isDeepStrictEqual(expectedConnection.tools, nextTools.tools) ||
          !isDeepStrictEqual(
            expectedConnection.toolListHints,
            nextTools.hints,
          ))) ||
      (nextResources !== undefined &&
        !isDeepStrictEqual(expectedConnection.resources, nextResources)) ||
      (nextPrompts !== undefined &&
    ...
    if (nextTools !== undefined) {
      ...
      expectedConnection.toolsRevision =
        (expectedConnection.toolsRevision ?? 0) + 1;
    }
  ...
  private async createConnection(
    ...
    const traceWriter = tracingEnabled
      ? (this.traceWriter ??= createMcpTraceWriter(
          this.defaultCwd,
          this.traceSettings ?? {},
        ))
      : undefined;
    ...
    if (definition.command) {
      ...
      let args = (definition.args ?? []).map((argument) =>
        interpolateEnvVars(argument),
      );
      ...
      if (command === "npx" || command === "npm") {
        ...
        if (resolved) {
          ...
          args = resolved.isJs
            ? [resolved.binPath, ...resolved.extraArgs]
            : resolved.extraArgs;
          logger.debug(
            `${name} resolved to ${resolved.binPath} (skipping npm parent)`,
          );
        }
      }
      throwIfAborted(signal);
  ...
  private async enrichHttpConnectionError(
    ...
    try {
      ...
      return new Error(`${originalMessage} — probe: ${probe.classification}`, {
        cause: error,
      });
    } catch {
  ...
  private createClient(
        ...
        listChanged: {
          ...
          resources: {
            onChanged: (
              ...
              this.handleResourcesListChanged(
                serverName,
                client,
                error,
                resources,
    ...
    if (this.elicitationConfig) {
      registerElicitationHandler(client, {
        ...
        serverName,
        onUrlAccepted: (elicitationId) =>
          this.rememberUrlElicitation(serverName, elicitationId),
      });
  ...
  private handleToolsListChanged(
    ...
    if (error) {
      logger.debug(
        `MCP: tools/list_changed refresh failed for ${serverName}: ${error.message}`,
      );
      return;
    }
    if (!tools) return;
  ...
  private handlePromptsListChanged(
    ...
    if (error) {
      logger.debug(
        `MCP: prompts/list_changed refresh failed for ${serverName}: ${error.message}`,
      );
      return;
    }
    if (!prompts) return;
  ...
  private async connectHttpClient(
    ...
    const commandBearer =
      definition.bearerToken?.startsWith("!") &&
      !definition.bearerToken.startsWith("!!")
    ...
    if (definition.auth === undefined && supportsOAuth(definition)) {
      try {
        implicitStoredAuth = inspectAuthForUrl(
          serverName,
          serverUrl,
          this.authStorageOptions,
        );
    ...
    const hasImplicitStoredTokens =
      implicitStoredAuth?.status === "present" &&
    ...
    for (;;) {
      ...
      if (
        authState.status === "implicit-deferred" &&
        isUnauthorizedHttpError(result.error)
      ) {
        authState = {
          status: "implicit-challenged",
  ...
  async readResource(
    ...
    try {
      ...
      return await connection.client.readResource(
        { uri },
        refreshRead
          ? { ...requestOptions, cacheMode: "refresh" }
          : requestOptions,
      );
    } finally {
      this.decrementInFlight(name);
  ...
  async close(name: string): Promise<void> {
    ...
    this.connectAttempts
      .get(name)
      ?.abort(new Error(`MCP connection ${name} was closed`));
    this.pendingMetadataPublications.delete(name);
  ...
  async closeAll(): Promise<void> {
    ...
    for (const name of names) {
      this.closeGenerations.set(
        name,
        (this.closeGenerations.get(name) ?? 0) + 1,
      );
      this.connectAttempts
        .get(name)
        ?.abort(new Error(`MCP connection ${name} was closed`));
    ...
    const results = await Promise.allSettled(
      currentNames.map((name) => this.close(name)),
    );
    ...
    const lateResults = await Promise.allSettled(
      lateNames.map((name) => this.close(name)),
    );
