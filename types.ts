 ...
 */
export function extractUiPromptText(
 ...
 */
export function parseUiPromptHandoff(
 prompt: string,
): UiPromptHandoff | undefined {
...
export const MCP_TOOL_APPROVAL_REQUEST_EVENT =
 "pi-mcp-adapter:tool-approval-request" as const;
...
export interface ServerCacheEntry {
 ...
 instructions?: string;
 /** Server-level hints from the aggregated tools/list result (MCP 2026-07-28). */
 ...
 */
function sanitizeServerPrefix(
 serverName: string,
 preserveProviderValid = true,
): string {
 const validCharacters = preserveProviderValid
  ? /^[A-Za-z0-9_-]$/
  : /^[A-Za-z0-9]$/;
...
export function formatPromptCommandName(
 ...
 const serverPart =
  getServerPrefix(serverName, prefix) ||
  sanitizeServerPrefix(serverName) ||
  "server";
...
export function getToolNameCandidates(
 ...
 if (includeLegacy) {
  ...
  candidates.add(
   formatToolName(toolName, serverName, prefix).replace(/-/g, "_"),
  );
  candidates.add(
   formatToolName(toolName, serverName, "server").replace(/-/g, "_"),
  );
  candidates.add(
   formatToolName(toolName, serverName, "short").replace(/-/g, "_"),
  );
  candidates.add(
   formatToolName(toolName, serverName, "mcp").replace(/-/g, "_"),
  );
 }
 return candidates;
}
...
export interface ToolSelectorCandidateIndex {
 ...
 readonly additionalCurrentCandidatesByToolName?: ReadonlyMap<
  string,
  ReadonlySet<string>
 >;
}
...
export function createToolSelectorCandidateIndex(
 ...
 additionalCurrentCandidatesByToolName?: ReadonlyMap<
  string,
  ReadonlySet<string>
 >,
 ...
 return {
  ...
  ...(additionalCurrentCandidatesByToolName
   ? { additionalCurrentCandidatesByToolName }
   : {}),
 };
}
...
export type ToolSelectorCandidateContext =
 | Set<string>
 | ToolSelectorCandidateIndex;
...
function indexHasOtherCurrentMatch(
 ...
 const additionalCandidates =
  index.additionalCurrentCandidatesByToolName?.get(toolName);
 const hasCandidate = (candidate: string): boolean =>
  index.allCurrentCandidates.has(candidate) ||
  additionalCandidates?.has(candidate) === true;
...
function matchesToolSelector(
 ...
 if (!Array.isArray(patterns) || patterns.length === 0) return false;
 const currentCandidates = getToolNameCandidates(
  toolName,
  serverName,
  prefix,
  false,
 ...
 if (!otherCurrentCandidates)
  return matchesToolPattern(
   getToolNameCandidates(toolName, serverName, prefix),
   patterns,
  );
